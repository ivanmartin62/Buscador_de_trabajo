...
class ConcursarScraper(BaseScraper):
    ...
    def parse(self, raw_data: Any) -> list[OfertaEmpleo]:
        ...
        tarjetas_procesadas: set[int] = set()
        for indicador in soup.find_all("span"):
            ...
            if identificador in tarjetas_procesadas:
                continue
            tarjetas_procesadas.add(identificador)
...
def _normalizar_tarjeta(
    ...
    url_original = _url_oficial(str(enlace["href"])) if enlace else URL_CONCURSAR
...
def _url_oficial(href: str) -> str:
    """Acepta enlaces web o relativos y descarta esquemas ejecutables."""
    url = urljoin(URL_CONCURSAR, href.strip())
    return url if url.startswith(("http://", "https://")) else URL_CONCURSAR
